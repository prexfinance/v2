...
contract MockBNBPricePrediction is Ownable, Pausable {
    ...
    modifier onlyAdmin() {
        require(msg.sender == adminAddress, "BNBPricePrediction: admin only function");
        _;
    }

    modifier onlyOperator() {
        require(msg.sender == operatorAddress, "BNBPricePrediction: operator only function");
        _;
    }

    modifier onlyAdminOrOperator() {
        require(msg.sender == adminAddress || msg.sender == operatorAddress, "BNBPricePrediction: admin | operator only function");
        _;
    }
    ...
    function betBear(uint256 _amount) external payable whenNotPaused notContract {
        ...
        require(BUSD.balanceOf(msg.sender) >= _amount, "Insufficient balance of BUSD");
    ...
    function _getPriceFromOracle(bool isLock) internal returns (int256) {
        ...
        (uint80 roundId, int256 price, , uint256 timestamp, ) = oracle.latestRoundData(isLock);
